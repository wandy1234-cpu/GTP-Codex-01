...
- `--backup-size 10`（替补池数量）
- `--watch-size 10`（观察池数量）
- `--sector-max-holdings 2`（正式池单行业上限）
- `--stop-loss 0.07` / `--trailing-stop 0.08` / `--max-hold-days 10` / `--signal-exit-rank 20`（退出规则参数）
- `--daily-report-md report.md` / `--daily-report-json report.json`（导出日报）
...
- `--use-liquidity-factor`（默认开启：Amihud 流动性因子）
...
- 支持“是否考虑流动性因子（Amihud）”下拉（默认：是）
- Web 页不再显示“外资因子 CSV 路径”输入框；开启因子时会自动读取项目目录下 `ib_top5_factor.csv`
- 输出中会显示每个 TopN 标的的主要因子贡献（Top3）
- 输出中增加：市场状态（regime/temperature/仓位建议）、正式池/替补池/观察池
- `请求超时(秒)` / `重试次数` 可视化配置（解决 live 模式超时问题）
- 可视化配置 Walk-forward 参数：`WF 训练天数/测试天数/步长天数`
- Walk-forward 默认训练窗口建议 252 天（程序会在样本不足时自动收缩窗口，避免“开启后无结果”）
- Web 端 Walk-forward 为避免超时，默认以 `horizon=5` 做快速回测展示
- Web 默认关闭 Walk-forward（先快速出主结果）；需要稳定性回测时再手动开启
...
- 因子算法升级：外资投行因子改为“稳健标准化 + 短中期趋势融合”；市场情绪/全球宏观因子改为“滚动稳健归一 + EMA 平滑”
- 新增 Amihud 流动性因子（指数增强常用），并提供独立开关 `--use-liquidity-factor`
...
- 新增市场状态识别（risk_on/neutral/risk_off）与仓位区间建议
