...
<body>
  ...
  <div class="card">
    ...
    <div class="row">
      ...
      <div>
        <label>考虑流动性因子（Amihud）</label>
        <select id="use_liquidity_factor">
          <option value="1">是（默认）</option>
          <option value="0">否</option>
        </select>
      </div>
...
async function run() {
  ...
  const payload = {
    ...
    use_liquidity_factor: document.getElementById('use_liquidity_factor').value,
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        use_liquidity_factor = str(payload.get("use_liquidity_factor", "1"))
        ...
        if use_liquidity_factor == "1":
            cmd.append("--use-liquidity-factor")
        else:
            cmd.append("--no-use-liquidity-factor")
